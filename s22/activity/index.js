...
let adventurer = {
    ...
    attackMonster: function(monster) {
        // Subtract the adventurer's attack power from the monster's health
        monster.health -= this.attack;
        if (monster.health <= 0) {
            // Drop loot
            let loot = monster.dropLoot();
            return `${this.name} attacks ${monster.name}! ${monster.name} has been defeated! ${loot}`;
        }
...
function Monster(name, level, health, loot) {
    ...
    this.dropLoot = function() {
        ...
        return `${this.name} drops ${this.loot}. ${adventurer.name}'s inventory: ${adventurer.inventory}`;
    };
}
